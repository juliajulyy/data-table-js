const SortData = (function () {
  ...
  const sortWorkers = (key) => {
    const workers = GetLocalWorker.getFilterWorkers();
  ...
  const setState = (item, key, operate = true) => {
    ...
    if ((tableHeaderStates[key] === 'initial') || (tableHeaderStates[key] === 'desc')) {
      ...
      DisplayWorkers.displayWorkers(sortWorkers(key), operate);
    } else if (tableHeaderStates[key] === 'asc') {
      item.src = 'img/desc.png';
      DisplayWorkers.displayWorkers(sortWorkers(key).reverse(), operate);
